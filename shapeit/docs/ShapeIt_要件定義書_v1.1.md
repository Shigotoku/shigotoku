...
## 5.1 エンティティ関係の基本
> **最重要：Feedback（生の気づき）と Issue（開発・改善単位）を分離する。複数Feedback → 1 Issue を標準とする。同一画面の複数 Issue は Fix Pack（派生ビュー）として束ね、エンジニアの1作業単位とする。**
概念関係：Organization → Workspace（任意）→ Project → Feedback → feedback_issue_links → Issue。IssueはRelease、Assignee、Product Area、AI Analysis、Activity等を持つ。Fix Pack は pageKey による派生集約（MVP〜V1は永続化不要）。
