...
| Changelog | 強化 | 公開/社内 visibility · 編集 |
| Feature flags | 強化 | ルートゲート + Nav 連動 |
| Ideas / Roadmap | 強化 | ARR・ソート · Unassigned · ←→ |
| Hotkeys / Search | 強化 | 主要画面ショートカット · Changelog/ナビ検索 |
| Board a11y | 強化 | フォーカス + ←→ 列移動 |
| Fix Packs | 強化 | URL集約 · サブパック分割 · 作業メタ(担当/PR) · 一括プロンプト · Closed Loop |
| Members / Projects | stub | 招待キュー · 切替 |
| 通知 | 強化 | Due / Assignee / @mention / SLA |
| CSV | 強化 | プレビュー → 取込 |
...
1. デモ開始 → 設定でリセット再シード
2. Board カード選択 → ←→ · Changelog 公開切替
3. Ideas で ARR · Roadmap Unassigned
4. コメントに `@tokunaga`

## 外部依存で未着手

Email 実送信、SSO/SAML、本課金、専用 GCP、Session replay 本実装
