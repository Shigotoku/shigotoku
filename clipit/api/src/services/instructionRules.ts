...
/** クリック要素・画面名から手順タイトルを生成 */
export function buildStepTitle(step: StepInput, order: number): string {
  const raw = (step.elementText || step.title || '').trim().replace(/^【強制キャプチャ】/, '');
  if (raw && !/^手順 \d+$/.test(raw) && !/^強制キャプチャ \d+$/.test(raw)) {
    const short = raw.slice(0, 36);
    return short.length < raw.length ? `${short}…` : short;
  }
  const page = step.pageTitle?.trim();
  if (page) return `${page}での操作`;
  return `手順 ${order}`;
}

/** 取り込み直後のマニュアルタイトル候補 */
export function suggestManualTitle(steps: StepInput[]): string | null {
  const page = steps.find((s) => s.pageTitle?.trim())?.pageTitle?.trim();
  if (page) return `${page}の操作手順`;
  const el = steps.find((s) => s.elementText?.trim() && !/^手順 \d+$/.test(s.elementText.trim()))?.elementText?.trim();
  if (el) {
    const short = el.slice(0, 28);
    return `${short}${el.length > 28 ? '…' : ''}の手順`;
  }
  if (steps.length > 0) return `${steps.length}手順の操作マニュアル`;
  return null;
}
