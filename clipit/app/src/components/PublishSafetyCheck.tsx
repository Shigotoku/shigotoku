...
export default function PublishSafetyCheck({ checked, onChange, compact, editHref }: Props) {
  return (
      ...
      {!compact && (
        <p className="mt-2 text-sm text-amber-800">
          ...
          {editHref ? (
            <>
              {' '}
              <Link to={editHref} className="font-semibold underline hover:text-amber-950">
                編集画面
              </Link>
              の手順詳細 →「マスキング」で黒塗り・モザイクできます。
            </>
          ) : (
            ' 編集画面の手順詳細 →「マスキング」で黒塗り・モザイクできます。'
          )}
        </p>
      )}
