...
export default function DashboardPage() {
  ...
  return (
    <>
      ...
      {!loading && manuals.length === 0 && !demoMode && (
        <div className="mx-6 mt-2">
          <EmptyState
            icon={<FilePlus2 size={40} />}
            title="最初のマニュアルを作りましょう"
            description="スクショをアップロードするだけなら、拡張なしですぐ試せます。作成後は共有URLでスタッフに渡せます。"
            actions={[
              { label: "スクショから作る（おすすめ）", to: "/manuals/new/screenshots", primary: true },
              { label: "テンプレートから", to: "/templates" },
            ]}
          />
        </div>
      )}
