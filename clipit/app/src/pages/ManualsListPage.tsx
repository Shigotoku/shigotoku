...
export default function ManualsListPage() {
  ...
  const filteredManuals = useMemo(() => {
    let list = manuals;
    if (folderFilter === "none") list = list.filter((m) => !m.folderId);
    else if (folderFilter !== "all") list = list.filter((m) => m.folderId === folderFilter);
    const q = searchQuery.trim().toLowerCase();
    if (q) list = list.filter((m) => m.title.toLowerCase().includes(q));
    return list;
  }, [manuals, folderFilter, searchQuery]);
    ...
    <>
      ...
      <div className="px-6">
        <input
          ...
          className="w-full max-w-md rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-400 focus:outline-none focus:ring-2 focus:ring-primary-100"
        />
      </div>
            ...
            {loading ? (
              ...
              searchQuery.trim() ? (
                <p className="px-5 py-12 text-center text-sm text-slate-500">
                  「{searchQuery}」に一致するマニュアルはありません。
                </p>
              ) : (
                <div className="p-5">
                  <EmptyState
                    title="マニュアルがまだありません"
                    description="スクショから始めるのがいちばん簡単です。テンプレートや操作記録も選べます。"
                    actions={[
                      { label: "スクショから作る", to: "/manuals/new/screenshots", primary: true },
                      { label: "作り方を選ぶ", to: "/manuals/new" },
                    ]}
                  />
                </div>
              )
