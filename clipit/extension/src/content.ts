...
window.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'CLIPIT_PING') {
    window.postMessage({ type: 'CLIPIT_PONG' }, event.origin);
    return;
  }
  if (event.data?.type === 'CLIPIT_SYNC') {
    chrome.storage.local.set({
      ...
      generateAllWithAi: Boolean(event.data.generateAllWithAi),
    });
    window.postMessage({ type: 'CLIPIT_SYNC_OK', manualId: event.data.manualId }, event.origin);
    return;
  }
