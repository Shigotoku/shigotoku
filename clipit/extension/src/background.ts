...
async function recordTabSwitchBoundary(tabId: number) {
  if (!recording || paused || recordingWindowId == null) return;
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!isRecordingWindow(tab.windowId) || !isInjectableUrl(tab.url)) return;
    const dataUrl = await captureStepScreenshot(tab.windowId!, tabId);
    if (!dataUrl) return;
    steps.push({
      title: `画面: ${(tab.title || '新しいタブ').slice(0, 36)}`,
      elementText: `【タブ切替】${tab.title || tab.url || ''}`,
      elementRole: 'tab',
      pageTitle: tab.title || '',
      pageUrl: tab.url || '',
...
chrome.tabs.onActivated.addListener((info) => {
  ...
  if (recording && !paused && lastActiveTabId != null && lastActiveTabId !== info.tabId) {
    void recordTabSwitchBoundary(info.tabId);
  }
...
async function finishIngest(): Promise<{ ok: boolean; message?: string }> {
  const { manualId, idToken, apiBase, voiceTranscript, polishVoiceWithAi, generateAllWithAi } =
  ...
  try {
    const res = await fetch(`${base}/v1/manuals/${manualId}/ingest`, {
      ...
      body: JSON.stringify({
        ...
        ...(generateAllWithAi ? { generateAllWithAi: true } : {}),
      }),
    });
    ...
    await persistRecordingMeta();
    chrome.tabs.create({ url: `${base.replace(/\/api\/?$/, '') || 'https://app.clipit.shigotoku.com'}/manuals/${manualId}/edit?ingested=1${generateAllWithAi ? '&ai=1' : ''}` });
