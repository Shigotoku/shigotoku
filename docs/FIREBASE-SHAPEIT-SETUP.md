...
## Phase 6b — メンバー招待（リンク共有・無料運用）

**既定:** 自動メール送信は行いません。管理者が設定画面で招待リンクを作成し、Slack・Teams・チャットなどで手動共有します（追加費用なし）。

### 手順

1. 管理者でログイン → **設定** → メンバー
2. メンバーのメールアドレスを入力 → **招待リンクを作成**
3. 表示されたリンクを **コピー** してメンバーに送る
4. メンバーがリンクを開き、パスワードを設定して参加

招待中のメンバーは一覧に表示され、いつでも **コピー** でリンクを再取得できます（有効期限 7 日）。

### 任意: 自動メール送信

将来、Resend または SMTP を設定すれば自動送信も可能です（`shapeit/api/.env.example` 参照）。無料運用では不要です。
